...
> A real-time full-stack transportation management platform that enables commuters to track jeepneys/buses live while providing operators with fleet monitoring, analytics, ETA prediction, and operational insights.

---

## 📖 Overview

BUSINA is a full-stack web application developed as a Computer Science Capstone Project. It simulates (and, when hardware is available, ingests real) GPS pings from vehicles, streams them over MQTT, stores them in Supabase PostgreSQL, and exposes a REST API — including ETA calculations, geofencing, and bunching detection — consumed by a React frontend in real time via Socket.IO.
...
simulator.js / replaySImulator.js  --publish-->  HiveMQ Cloud (MQTT)  --subscribe-->  subscriber.js  -->  Supabase PostgreSQL
                                                                                                                |
                                                                                                                v
                                                                                  bunching.js (every 30s)  <--  server.js (REST API + Socket.IO)
                                                                                          |                         |
                                                                                          v                         v
                                                                                   bunching_alerts          Frontend (Commuter + Operator views)
                                                                                    (Supabase)
```
...
2. **`replaySImulator.js`** replays real multi-vehicle GPS data (from the AI Lead's `simulated_trips_roadsnapped.csv`) over MQTT — publishing all 10 vehicles across 5 routes at their real road-snapped positions, on a 1-second tick, for realistic testing (including bunching scenarios). Toggled on/off via the `ENABLE_REPLAY_SIM` environment variable — see [Recent Updates](#-recent-updates--changelog).
...
4. **`server.js`** exposes REST endpoints that read from Supabase, including computed ETAs, broadcasts live updates over Socket.IO, and starts the bunching detection monitor on a 30-second interval. Also requires `subscriber.js` directly so both run in a single process.
...
| Turf.js (`@turf/turf`) | Geofencing |
| HiveMQ Cloud | MQTT broker (serverless free tier) |
...
| `replaySImulator.js` | Replays real multi-vehicle road-snapped GPS data over MQTT for realistic testing, including bunching scenarios. Gated behind `ENABLE_REPLAY_SIM` |
...
| `server.js` | Express + Socket.IO REST API; starts bunching monitor and subscriber on startup; dynamic port + configurable CORS |
...
| `.env` | MQTT and Supabase credentials, plus deployment config (not committed) |
...
| GET | `/routes` | Route/geofence data |
...
**Socket.IO** — subscribe to live push updates without polling:
```javascript
...
**Supabase Realtime** (alternative) — subscribe directly to the `vehicles` table:
```javascript
...
# Deployment / production config (see Recent Updates below)
FRONTEND_URL=https://your-frontend-domain.vercel.app
ENABLE_REPLAY_SIM=true
PORT=            # leave unset in production — Render assigns this automatically
```
...
cd backend
npm install
node server.js       # starts server.js, which also starts subscriber + bunching monitor
```

Or run each piece separately for development:
```bash
node subscriber.js        # MQTT -> Supabase
node server.js            # REST API + Socket.IO on port 3000 + bunching monitor
...
### Backend deployment fixes (Render)

- **Fixed missing `socket.io` dependency** — the backend crashed on deploy with `Error: Cannot find module 'socket.io'` because the package was used in `server.js` but never added to `package.json`. It worked locally (already present in local `node_modules`) but failed on fresh installs, like Render's build process. Added `socket.io` as a proper declared dependency.

- **Dynamic port binding for production** — `server.js` previously hardcoded `const PORT = 3000`. Render (and most hosting platforms) assigns a port dynamically via `process.env.PORT`, so hardcoding a port is fragile in production. Now uses:
  ```js
  const PORT = process.env.PORT || 3000;
  ```
  This uses the platform-assigned port in production while still defaulting to `3000` locally.

- **Configurable CORS for both Express and Socket.IO** — CORS was hardcoded to `http://localhost:5173` only, which blocked both API requests and Socket.IO connections from the deployed Vercel frontend. Now supports the deployed frontend and local dev via environment-variable-driven origin config, applied consistently to:
  - Express (`app.use(cors(...))`)
  - Socket.IO (`new Server(server, { cors: {...} })`)

  This lets the same codebase work correctly in local development and production without code changes — just update `FRONTEND_URL` when the frontend domain changes.

- **Wired in `replaySImulator.js`** — the file existed in the codebase but was never actually invoked by `server.js`, so it had no effect when deployed. It's now conditionally required at startup, gated behind the `ENABLE_REPLAY_SIM` environment variable, so it can be toggled on for demos/testing (publishing simulated vehicle location data over MQTT) without affecting production behavior when real ESP32/hardware data is expected instead.

- **Added a root health-check route** — `GET /` now returns a friendly status message instead of Express's default "Cannot GET /", useful for quick sanity checks and uptime monitoring.

**Environment variables required for this update:**

| Variable | Purpose | Example |
|---|---|---|
| `FRONTEND_URL` | Frontend domain allowed to access the API and Socket.IO | `https://busina.vercel.app` |
| `ENABLE_REPLAY_SIM` | Toggles the fake vehicle data simulator on/off | `true` / `false` |
...
- MQTT credentials are split into publish-only and subscribe-only accounts (least privilege)
...
- Socket.IO only exposes operational (non-sensitive) data
- CORS is restricted to explicitly allowed origins in production
...
> **BUSINA** bridges commuters and transport operators through real-time vehicle tracking, intelligent analytics, and modern web technologies — a scalable foundation for smarter public transportation systems.
